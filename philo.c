...
pthread_mutex_t tenedores[N]; // Un mutex por tenedor
...
void eat(int id) {
    ...
    // Filósofos impares cambian el orden para evitar deadlock
    if (id % 2 == 0) {
        f[0] = id;
        f[1] = (id + 1) % N;
    } else {
    ...
    // Toma los tenedores con mutex
    pthread_mutex_lock(&tenedores[f[0]]);
    pthread_mutex_lock(&tenedores[f[1]]);
